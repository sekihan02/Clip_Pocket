...
Clip Pocket は Windows クリップボードにコピーされたテキストを監視し、メモリ上の一時リストに保持します。前にコピーした内容を貼り付けたいときは、Clip Pocket を開き、必要なら検索や並び替えで項目を探し、次に貼り付けたい項目をダブルクリックしてから貼り付け先で `Ctrl+V` します。

## スクリーンショット

![Clip Pocket メインウィンドウ](assets/clip-pocket-main.png)

画像は初期値の白設定を使った例です。色、透過率、文字サイズ、ウィンドウサイズは `Settings` / `設定` から変更できます。
...
- ウィンドウの透過率を変更可能
- 文字サイズを変更可能
- ウィンドウ端のドラッグ、または設定からウィンドウサイズを変更可能
...
- 言語
- 色
- 透過率（0〜100%。0%でも完全には見えなくならないよう、実表示では薄く残します）
- 文字サイズ
- ウィンドウ幅
- ウィンドウ高さ
- コピーの削除までの期間
...
Clip Pocket は通知領域に隠れて起動します。通知領域アイコンをダブルクリックするか、`Ctrl` を二回押すか、`ClipPocket.exe` をもう一度実行してください。
